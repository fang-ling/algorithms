...
/// element of `A[q + 1 ... r]`.
private
func partition<T: Comparable>(array: inout [T], _ p : Int, _ r : Int) -> Int {
    let x = array[r]
    var i = p - 1
    for j in p ..< r {
        if array[j] <= x {
...
/// .
private
func randomized_partition<T : Comparable> (array: inout [T],
                                           _ p : Int,
                                           _ r : Int) -> Int {
...
private func quicksort<T : Comparable>(array: inout [T], _ p : Int, _ r : Int) {
    if p < r {
